...
public static class HelpText {
    ...
    public static List<string> GetOptionsDetailsLines(Verb verb, uint consoleWidth = 80, uint leftSpace = 2,
        ...
        for (int iOption = 0; iOption < optionsDetailsText.Count; iOption++) {
            ...
            returnedLines.AddRange((
                ...
                SplitToMultiline(verb.Options[iOption].Description ?? string.Empty,
                    (int) consoleWidth - maxDetailsSize,
                    new string(' ', maxDetailsSize))
    ...
    ///     weird and unexpected line returns in some command prompts.
    /// </remarks>
    public static string GetVerbsDetails(Verb verb, uint consoleWidth = 80, uint leftSpace = 2,
        uint innerSpace = 2) {
        string outputString = "";
        int maxSubVerbSize = 0;

        foreach (Verb subVerb in verb.Verbs) {
            if (subVerb.Name != null && subVerb.Name.Length > maxSubVerbSize) {
        ...
        foreach (Verb subVerb in verb.Verbs) {
            outputString += new string(' ', (int) leftSpace) + subVerb.Name +
                            new string(' ', maxSubVerbSize - (subVerb.Name ?? string.Empty).Length + (int) innerSpace) +
                            SplitToMultiline(subVerb.Description ?? string.Empty,
